...
{
    [TestFixture]
    public class PlayerControllerAntiForgeryTests : PlayerControllerTestBase
    {
        [TestCase(nameof(PlayerController.Edit), new[] { typeof(BusinessLogic.Models.Player), typeof(BusinessLogic.Models.User.ApplicationUser) })]
        [TestCase(nameof(PlayerController.Delete), new[] { typeof(int), typeof(BusinessLogic.Models.User.ApplicationUser) })]
        public void ListedActionsUseHandledValidateAntiForgeryTokenForbiddenHtmlMode(string actionName, Type[] parameterTypes)
        {
            var actionMethod = GetPlayerActionMethod(actionName, parameterTypes);
        ...
        [Test]
        public void DeleteHandledAntiForgeryFailureReturns403AndSkipsDeletePlayer()
        {
            var actionMethod = GetPlayerActionMethod(nameof(PlayerController.Delete), typeof(int), typeof(BusinessLogic.Models.User.ApplicationUser));
            ...
            autoMocker.Get<IPlayerDeleter>().AssertWasNotCalled(mock => mock.DeletePlayer(Arg<int>.Is.Anything, Arg<BusinessLogic.Models.User.ApplicationUser>.Is.Anything));
        }

        [Test]
        public void DeleteUnauthenticatedRequestKeepsAuthorizeResultInsteadOfFallingIntoAntiForgeryHandling()
        {
            var actionMethod = GetPlayerActionMethod(nameof(PlayerController.Delete), typeof(int), typeof(BusinessLogic.Models.User.ApplicationUser));
        ...
        private AuthorizationContext CreateAuthorizationContext(MethodInfo actionMethod, bool isAuthenticated)
        {
            ...
            var actionDescriptor = new ReflectedActionDescriptor(actionMethod, actionMethod.Name, new ReflectedControllerDescriptor(typeof(PlayerController)));

            return new AuthorizationContext(controllerContext, actionDescriptor);
        }

        private static MethodInfo GetPlayerActionMethod(string actionName, params Type[] parameterTypes)
        {
            return typeof(PlayerController).GetMethod(actionName, parameterTypes);
        }
