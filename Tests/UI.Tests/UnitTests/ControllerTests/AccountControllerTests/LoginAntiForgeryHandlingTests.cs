...
{
    [TestFixture]
    public class LoginAntiForgeryHandlingTests : AccountControllerTestBase
    {
        [Test]
        public void LoginFormSetsTheOriginOnTheViewModel()
        {
            var result = accountControllerPartialMock.LoginForm("home") as PartialViewResult;

            Assert.That(result, Is.Not.Null);
            Assert.That(result.ViewName, Is.EqualTo(MVC.Account.Views.LoginForm));

            var model = result.Model as LoginViewModel;
            Assert.That(model, Is.Not.Null);
            Assert.That(model.Origin, Is.EqualTo("home"));
        }

        [Test]
        public void LoginHandledAntiForgeryFailureRedirectsBackToTheLoginPageAndSkipsActionExecution()
        {
            AssertHandledRetryLoginFailure(
                GetAccountActionMethod(nameof(AccountController.Login), typeof(LoginViewModel), typeof(string)),
                "login",
                "/Account/Login");
        }

        [Test]
        public void LoginHandledAntiForgeryFailureRedirectsBackToHomeAndSkipsActionExecution()
        {
            AssertHandledRetryLoginFailure(
                GetAccountActionMethod(nameof(AccountController.Login), typeof(LoginViewModel), typeof(string)),
                "home",
        ...
        [Test]
        public void ExternalLoginHandledAntiForgeryFailureRedirectsBackToRegisterAndSkipsActionExecution()
        {
            AssertHandledRetryLoginFailure(
                GetAccountActionMethod(nameof(AccountController.ExternalLogin), typeof(string), typeof(string)),
                "register",
                "/Account/Register");
        }

        private void AssertHandledRetryLoginFailure(MethodInfo actionMethod, string origin, string expectedUrl)
        {
            ...
            Assert.That(GetFailureMode(configuredAttribute), Is.EqualTo(AntiForgeryFailureMode.RetryLogin));

            var filterContext = CreateAuthorizationContext(origin);
            var testableAttribute = new TestableHandledValidateAntiForgeryTokenAttribute(AntiForgeryFailureMode.RetryLogin)
            {
                ShouldThrow = true
            };
        ...
        private AuthorizationContext CreateAuthorizationContext(string origin)
        {
