...
{
    [TestFixture]
    public class TotallyCategorizedAchievementTests
    {
        private RhinoAutoMocker<TotallyCategorizedAchievement> _autoMocker;
        private readonly int _playerId = 1;

        [SetUp]
        public void SetUp()
        {
            _autoMocker = new RhinoAutoMocker<TotallyCategorizedAchievement>();
        }
                                ...
                                BoardGameGeekGameDefinition = new BoardGameGeekGameDefinition
                                {
                                    Categories = new List<BoardGameGeekGameCategory>
                                    {
                                        new BoardGameGeekGameCategory
                                            {
                                                CategoryName = string.Format("CatName {0}", j)
                                            }
                                        }
                                }
                            }
