...
namespace BusinessLogic.Tests.IntegrationTests.LogicTests.PlayerRepositoryTests
{
    ...
    public class GetPlayerDetailsIntegrationTests : IntegrationTestBase
    {
        ...
        public void ItEagerlyFetchesPlayerGameResults()
        {
            ...
            {
                PlayerRepository playerRepository = new PlayerRepository(dbContext, userContextBuilder);
        ...
        public void ItSetsPlayerStatistics()
        {
            using (NemeStatsDbContext dbContext = new NemeStatsDbContext())
            {
                PlayerDetails playerDetails = new PlayerRepository(dbContext, userContextBuilder).GetPlayerDetails(testPlayer1.Id, 1);
        ...
        public void ItOnlyGetsTheSpecifiedNumberOfRecentGames()
        {
            using (NemeStatsDbContext dbContext = new NemeStatsDbContext())
            {
                ...
                PlayerDetails playerDetails = new PlayerRepository(dbContext, userContextBuilder).GetPlayerDetails(testPlayer1.Id, numberOfGamesToRetrieve);
